(function() {
	...
	app.run(function($rootScope, $state, $location, $growl, UserProfile, Modal, Box) {
	    $rootScope.$on( '$stateChangeStart', function(e, toState, toParams, fromState) {
		    ...
		    //cannot access states that require user to not be authenticated
				...
				$growl.box("Error", "Authorized users cannot view this page", {
					class: "danger"
				}).open();
		    	$state.go('home');
		    	e.preventDefault();
		    ...
		    //prevent user from getting to the upload page without a box id set (meaning user did not set a box to upload to)
		    	...
		    	$growl.box("Error", "Users must view a box in order to upload", {
					class: "danger"
				}).open();
		     	$state.go('home');
		     	e.preventDefault();
